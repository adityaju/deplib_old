...
from api.db_credentials import setu
...
def return_book1(mem_id, book_acn, remarks=""):
    wapas = setu.cursor()
    ...
    query2 = "update books set is_available = 1 where accession_number=\'"+str(book_acn)+"\';"
    print(query2)
    val = (remarks, mem_id, str(book_acn))

    wapas.execute(query,val)
    setu.commit()
    wapas.execute(query2)
    setu.commit()
    print(wapas)
